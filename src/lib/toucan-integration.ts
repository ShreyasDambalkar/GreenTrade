...
export class ToucanIntegrationService {
  ...
  async initialize(provider: JsonRpcProvider, signer: Signer) {
    try {
      ...
      console.log('âœ… Toucan Protocol client initialized successfully');
      return true;
    } catch (error) {
      console.error('âŒ Failed to initialize Toucan client:', error);
      return false;
    }
  }
  ...
  async getAvailableProjects(): Promise<ToucanProject[]> {
    if (!this.toucan) {
      console.warn('âš ï¸ Toucan client not initialized, returning mock data');
    ...
    try {
      ...
      console.error('âŒ Failed to fetch Toucan projects:', error);
      return this.getMockProjects();
    }
  }
  ...
  async redeemToTCO2(amount: string, poolToken: string = 'NCT'): Promise<string | null> {
    if (!this.toucan) {
      console.error('âŒ Toucan client not initialized');
      return null;
    }

    try {
      console.log(`ðŸ”„ Redeeming ${amount} ${poolToken} tokens to TCO2...`);
      ...
      console.log('âœ… Successfully redeemed to TCO2:', result);
      return (result as any).hash || 'mock-redeem-hash';
    } catch (error) {
      console.error('âŒ Failed to redeem to TCO2:', error);
      return null;
    }
  }
  ...
  async retireCredits(
    ...
    try {
      console.log(`ðŸ”¥ Retiring ${amount} TCO2 tokens for ${beneficiary}...`);
      ...
      console.log('âœ… Successfully retired carbon credits:', retirement);
      return retirement;
    } catch (error) {
      console.error('âŒ Failed to retire carbon credits:', error);
      return null;
    }
  }
  ...
  async getRetirementHistory(address: string): Promise<ToucanRetirement[]> {
    ...
    try {
      ...
      console.error('âŒ Failed to fetch retirement history:', error);
      return [];
    }
  }
  ...
  async offsetCarbon(
    ...
    try {
      console.log(`ðŸŒ± One-click carbon offset: ${amount} ${tokenToSwap}...`);
      ...
      console.log('âœ… Carbon offset completed successfully');
      return 'mock-offset-transaction-hash';
    } catch (error) {
      console.error('âŒ Failed to complete carbon offset:', error);
      return null;
    }
  }
  ...
  async getPoolTokenBalance(address: string, poolToken: string = 'NCT'): Promise<string> {
    ...
    try {
      ...
      console.error('âŒ Failed to get pool token balance:', error);
      return '0';
    }
  }
