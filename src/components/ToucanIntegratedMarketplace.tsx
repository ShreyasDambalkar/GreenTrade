  ...
  const handleBuyCredits = async (listing: MarketplaceListing) => {
    ...
    try {
      ...
      if (redeemHash) {
        ...
        alert('âŒ Purchase failed. Please try again.');
      }
    } catch (error) {
  ...
  const handleRetireCredits = async () => {
    ...
    try {
      const retirement = await toucanService.retireCredits(
        ...
        retireMessage || 'Carbon footprint offset via GreenTrade'
      );
      ...
      if (retirement) {
        ...
        alert('âŒ Retirement failed. Please try again.');
      }
    } catch (error) {
  ...
  const handleOffsetCarbon = async (amount: string) => {
    ...
    try {
      const offsetHash = await toucanService.offsetCarbon(
        amount,
        'USDC',
        'One-click carbon offset via GreenTrade'
      );

      if (offsetHash) {
        alert(`ðŸŒ± Successfully offset ${amount} USD worth of carbon!\\nTransaction: ${offsetHash}`);
      } else {
        alert('âŒ Offset failed. Please try again.');
      }
    } catch (error) {
