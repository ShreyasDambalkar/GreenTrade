...
// import { useActiveAccount } from 'thirdweb/react'; // Removed to fix error
...
export default function ModernMarketplace() {
  ...
  const account = address ? { address } : null; // Compatibility shim
  ...
  useEffect(() => {
    if (activeTab === 'portfolio' && account) {
      fetchUserPortfolio();
    }
  }, [activeTab, account]);
  ...
  const fetchUserPortfolio = async () => {
    ...
    const localTrades = JSON.parse(localStorage.getItem('userTrades') || '[]').filter(
      (t: any) => t.user_address === account.address
    );

    // Merge trades
    const allTrades = [...(supabaseTrades || []), ...localTrades];

    // Aggregate holdings
    const holdings: Record<string, number> = {};
    allTrades.forEach((trade: any) => {
      if (!holdings[trade.symbol]) holdings[trade.symbol] = 0;
      if (trade.side === 'buy') holdings[trade.symbol] += Number(trade.quantity);
      else holdings[trade.symbol] -= Number(trade.quantity); // handle sells if any
    });

    // Convert to array
    const assets = Object.entries(holdings)
      .map(([symbol, balance]) => {
        // Find price from mock data or listings (simplified)
        // For now, we mimic price from realProjects if possible, or default
        const project = realProjects.find(p => {
          if (symbol === 'GOLD') return p.methodology === 'Gold Standard';
          if (symbol === 'REDD') return p.projectType === 'Forest Conservation';
          return false;
        });
        const price = project ? parseFloat(project.pricePerTonne) : 0.015; // default avg

        return {
          symbol,
          balance,
          value: balance * price,
          name: symbol === 'GOLD' ? 'Gold Standard Credits' : (symbol === 'REDD' ? 'REDD+ Forest Credits' : 'Verified Carbon Units')
        };
      })
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      // Rough conversion: 1 ETH = ~250,000 INR (Demo rate)
      const ethToInrRate = 250000;
      const totalEth = amount * parseFloat(selectedListing.pricePerTonne);
      const totalInr = Math.round(totalEth * ethToInrRate);

      const response = await fetch('/api/payment/razorpay', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: totalInr, currency: 'INR' }),
      });
      ...
      const options = {
        ...
        name: "GreenTrade Carbon",
        description: `Purchase ${amount} tonnes of ${selectedListing.projectName}`,
        image: "https://example.com/your_logo", // You can substitute your logo
        order_id: data.order.id,
        handler: async function (response: any) {
          // 3. Payment Success Handler
          console.log("Payment Successful", response);

          // Record trade and show success (reuse existing logic logic slightly modified)
          const symbol = selectedListing.methodology === 'Gold Standard' ? 'GOLD' : (selectedListing.methodology === 'VCS' ? (selectedListing.projectType === 'Forest Conservation' ? 'REDD' : 'VCU') : 'CCX');

          // 1. Supabase
          try {
            await supabase.from('trades').insert({
              user_address: account?.address || 'guest_user',
              symbol: symbol,
              side: 'buy',
              price: parseFloat(selectedListing.pricePerTonne),
              quantity: amount,
        ...
        prefill: {
          // Razorpay will prompt the user for these details
        },
        theme: {
  ...
  const executePurchase = async () => {
    ...
    try {
      ...
      // Map project type/methodology to portfolio symbols
      let symbol = 'CCX';
      if (selectedListing.methodology === 'Gold Standard') symbol = 'GOLD';
      else if (selectedListing.methodology === 'VCS') {
        if (selectedListing.projectType === 'Forest Conservation') symbol = 'REDD';
        else symbol = 'VCU';
      }

      // Record trade in Supabase for Portfolio visibility (Best Effort)
      try {
        ...
        if (tradeError) console.error('Failed to record trade in Supabase:', tradeError);
      } catch (err) {
        console.warn('Supabase error, falling back to local storage', err);
      }

      // ALWAYS Save to LocalStorage (Reliable Fallback for Guest/Demo)
      const existingTrades = JSON.parse(localStorage.getItem('userTrades') || '[]');
      existingTrades.push({
        user_address: account.address,
        symbol: symbol,
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
          ...
          <div className="bg-zinc-900/50 backdrop-blur-sm rounded-2xl border border-zinc-800 p-6">
            <div className="flex flex-wrap gap-2 mb-6">
              <button
                ...
                className={`px-6 py-3 rounded-xl transition-all duration-200 flex items-center gap-2 ${activeTab === 'marketplace'
              ...
              <button
                ...
                className={`px-6 py-3 rounded-xl transition-all duration-200 flex items-center gap-2 ${activeTab === 'portfolio'
                  ? 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white'
                  : 'bg-zinc-800/50 text-zinc-300 hover:bg-zinc-700/50'
                  }`}
              >
              ...
              <button
                ...
                className={`px-6 py-3 rounded-xl transition-all duration-200 flex items-center gap-2 ${activeTab === 'mint'
                  ? 'bg-gradient-to-r from-indigo-600 to-violet-600 text-white'
                  : 'bg-zinc-800/50 text-zinc-300 hover:bg-zinc-700/50'
                  }`}
              >
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-4 pb-16">
          <div className="bg-zinc-900/50 backdrop-blur-sm rounded-2xl border border-zinc-800 p-8">
            ...
            {!account ? (
              ...
              <div className="text-center py-12">
                <Leaf className="w-16 h-16 text-zinc-600 mx-auto mb-4" />
                <p className="text-zinc-400 text-lg">No carbon credits found</p>
                <p className="text-zinc-500 text-sm mt-2">Purchase credits from the marketplace to see them here.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {portfolioAssets.map((asset) => (
                  <div key={asset.symbol} className="bg-zinc-800/50 border border-zinc-700 rounded-xl p-6 relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="bg-emerald-500/20 text-emerald-400 text-xs font-bold px-2 py-1 rounded-full border border-emerald-500/30">
                          Verified
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <p className="text-2xl font-mono text-white">{asset.balance.toLocaleString()} <span className="text-sm text-zinc-500">tonnes</span></p>
                        </div>
                        <div>
                          <p className="text-zinc-500 text-xs uppercase tracking-wider">Value (Approx)</p>
                          <p className="text-lg font-mono text-zinc-300">{asset.value.toFixed(4)} ETH</p>
                        </div>
                      </div>

                      <div className="mt-6 pt-4 border-t border-zinc-700/50 flex gap-2">
                        <button
                          onClick={() => {
                            const amount = prompt(`How many tonnes of ${asset.symbol} to retire?`, '1');
                            if (amount) alert(`Retired ${amount} tonnes of ${asset.symbol}. Certificate generated! (Demo)`);
                          }}
                          className="flex-1 bg-zinc-700 hover:bg-zinc-600 text-white text-sm py-2 rounded-lg transition-colors"
                        >
                          Retire
                        </button>
                        <button
                          onClick={async () => {
                            const amountStr = prompt(`How many tonnes of ${asset.symbol} to sell?`, '1');
                            if (!amountStr) return;
                            const qty = parseFloat(amountStr);
                            if (isNaN(qty) || qty <= 0 || qty > asset.balance) {
                            ...
                            try {
                              await supabase.from('trades').insert({
                                ...
                                symbol: asset.symbol,
                                side: 'sell',
                                price: asset.value / asset.balance, // Approx price
                                quantity: qty,
                                created_at: new Date().toISOString()
                              });
                            ...
                            alert(`Successfully sold ${qty} tonnes of ${asset.symbol}`);
                            fetchUserPortfolio(); // Refresh view
                          }}
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-emerald-600 to-emerald-600 hover:from-emerald-500 hover:to-emerald-500 disabled:from-zinc-600 disabled:to-zinc-600 disabled:cursor-not-allowed text-white py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2"
                  >
                    <Factory className="w-4 h-4" />
                    Mint Carbon Credits
                  </button>
      ...
      {purchaseModalOpen && selectedListing && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-zinc-900 rounded-2xl border border-zinc-700 p-8 max-w-md w-full shadow-2xl">
              ...
              {purchaseAmount && (
                <div className="mt-2 text-sm text-zinc-300 bg-zinc-800/30 rounded-lg p-3">
                  ...
                  <div className="flex justify-between text-emerald-400">
                    ...
                    <span className="font-semibold">₹{(parseFloat(purchaseAmount) * parseFloat(selectedListing.pricePerTonne) * 250000).toLocaleString()}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <ShoppingCart className="w-4 h-4" />
                {purchasing ? 'Processing...' : 'Pay with Crypto (ETH)'}
              </button>
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-zinc-900 px-2 text-zinc-500">Or pay with Fiat</span>
                </div>
              </div>
              ...
              <button
                ...
                disabled={purchasing || !purchaseAmount}
                className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <CreditCard className="w-4 h-4" />
                Pay with INR (UPI/Card)
              </button>
            </div>
