...
async function main() {
  ...
  console.log('🪙 Deploying GreenTradeToken (ERC20)...');
  const GreenTradeToken = await ethers.getContractFactory('GreenTradeToken');
  const greenTradeToken = await GreenTradeToken.deploy();
  await greenTradeToken.waitForDeployment();
  const erc20Address = await greenTradeToken.getAddress();
  ...
  const project1Tx = await greenTradeToken.addProject(
  ...
  const project2Tx = await greenTradeToken.addProject(
  ...
  await greenTradeToken.mintFromProject(
    deployer.address,
    ethers.parseEther('100'), // 100 GTT tokens
    0 // Wind farm project
  );

  await greenTradeToken.mintFromProject(
    deployer.address,
    ethers.parseEther('75'), // 75 GTT tokens
  ...
  console.log('   Symbol: GTT (GreenTrade Token)');
  console.log('   Total Supply: 175 GTT');
  console.log('   Projects: 2 sample projects created');
  console.log('');
