...
task("deploy-mock", "Deploy mock contracts for testing")
    .setAction(async (taskArgs, hre) => {
        ...
        // Deploy second contract (GreenTradeToken mock)
        console.log('💎 Deploying GreenTradeToken mock...');
        const greenTrade = await StorageContract.deploy();
        await greenTrade.waitForDeployment();
        const greenTradeAddress = await greenTrade.getAddress();
        console.log(`✅ GreenTradeToken deployed at: ${greenTradeAddress}`);
        ...
        updateEnv('NEXT_PUBLIC_GREENTRADE_TOKEN_ADDRESS', greenTradeAddress);
        updateEnv('NEXT_PUBLIC_CONTRACT_ADDRESS', carbonCreditAddress);
        updateEnv('NEXT_PUBLIC_ERC20_ADDRESS', greenTradeAddress);
        ...
        console.log(`💎 GreenTradeToken: ${greenTradeAddress}`);
